...
| 순서 | API | 용도 |
|------|-----|------|
| 1 | `POST /books` | 포토북 객체 생성 (bookUid 발급) |
| 2 | `POST /books/{bookUid}/cover` | 앞·뒷표지 이미지 및 템플릿 설정 |
| 3 | `POST /books/{bookUid}/contents` | 내지 사진 1장씩 추가 (최소 24회 반복) |
| 4 | `POST /books/{bookUid}/finalization` | 포토북 확정 (이후 변경 불가) |
| 5 | `POST /orders/estimate` | 실제 견적 조회 — 크레딧 충분 여부 확인 후 주문 진행 |
| 6 | `POST /orders` | 주문 생성 및 배송 정보 등록 (`Idempotency-Key` 포함) |
...
**왜 여행 포토북인가?**
Book Print API의 핵심 가치는 "디지털 콘텐츠를 실물 책으로 만드는 것"입니다. 여행 사진은 스마트폰에 묻혀 있기 쉽지만 포토북으로 만들면 영구적으로 간직할 수 있습니다. 1인 파트너 관점에서 가장 명확한 수요와 완결된 API 플로우를 가진 주제라고 판단했습니다.
...
<!-- 스크린샷 섹션 — 추후 이미지 첨부 예정 -->

| 화면 | 설명 |
|------|------|
| Step 1 입력 | 표지/내지 업로드 및 기본 정보 입력 |
| 24장 충족 | 내지 배지가 초록으로 전환, 주문 버튼 활성화 |
| API 견적 표시 | finalization 후 실제 견적 금액으로 카드 업데이트 |
| 주문 완료 화면 | orderUid 표시, 새 포토북 만들기 버튼 |
